...
# Me
from perfis.models import CATEGORIAS, ESTILOS, MAUTORIA
from profiles.models import ESTADOS, Profile
import os
BLANK=(('',''),)
...
class LoginForm(GroupForm):
    ...
    remember = forms.BooleanField(
        label = _("Mantenha-me conectado"),
        help_text = _(u"Se marcado você permanecerá conectado por 3 semanas"),
        required = False
    )
...
class SignupForm(GroupForm):
    ...
    # Me
    pnome=forms.CharField(label=_(u'Primeiro nome'), max_length=30, widget=forms.TextInput(), required=False)
    unome=forms.CharField(label=_(u'Último nome'), max_length=30, widget=forms.TextInput(), required=False)
    nartistico=forms.CharField(label=_(u'Nome artístico'), max_length=56, widget=forms.TextInput(), required=False)
    cidade=forms.CharField(label=_(u'Cidade'), max_length=56, widget=forms.TextInput(), required=False)
    estado=forms.CharField(label=_('Estado'), max_length=2,required=False, widget=forms.Select(choices=BLANK+ESTADOS))
    ...
    def save(self, request=None):
        ...
        # Me
        # User
        new_user.first_name=self.cleaned_data["pnome"]
        new_user.last_name=self.cleaned_data["unome"]
        new_user.save()
        # Profile
        perfil=Profile.objects.get(user=new_user.id)
        perfil.name=str(new_user.first_name)+' '+new_user.last_name
        perfil.nartistico=self.cleaned_data["nartistico"]
        perfil.location=self.cleaned_data["cidade"]
        try:
            perfil.estado=Profile.objects.get(estado=self.cleaned_data["estado"])
        except:
            perfil.estado_id=8# ES
        perfil.data_nascimento=self.cleaned_data["data_nascimento"]
        perfil.save()        
        
        # Send email
        os.system('~/apps_wsgi/dev/send.sh')
