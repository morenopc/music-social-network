...
def login(request, **kwargs):
    ...
    if request.method == "POST" and not url_required:
        ...
        if form.is_valid():
            ...
            muc=None
            ava=None
            user_perfil=form.user
            try:
                muc=Musicas.objects.filter(perfil__user=user_perfil.id)
                ava=Avatar.objects.filter(user=user_perfil.id)
            except:
                if not muc: 
                    return redirect('/dev/cadastro/musicas/')
                if not ava:
                    return redirect('/dev/cadastro/fotos/')
