...
if EmailAddress:
    class JoinRequestForm(forms.Form):
        ...
        message = forms.CharField(label="Mensagem", required=False, widget=forms.Textarea(attrs = {'cols': '30', 'rows': '5'}))
        ...
        def clean_email(self):
            ...
            if self.existing_users:
                raise forms.ValidationError(u"Esse endereço de Email já foi usado.")
            return self.cleaned_data["email"]
        
        def save(self, user):
            join_request = JoinInvitation.objects.send_invitation(user, self.cleaned_data["email"], self.cleaned_data["message"])
            user.message_set.create(message="Convite envia para %s" % join_request.contact.email)
            return join_request
...
class InviteFriendForm(UserForm):
    ...
    message = forms.CharField(label="Mensagem", required=False, widget=forms.Textarea(attrs = {'cols': '20', 'rows': '5'}))
    ...
    def clean_to_user(self):
        ...
        try:
            ...
            raise forms.ValidationError(u"Usuário desconhecido.")
    ...
    def clean(self):
        ...
        if previous_invitations_to.count() > 0:
            raise forms.ValidationError(u"Você já pediu a amizade de %s" % to_user.username)
        # check inverse
        ...
        if previous_invitations_from.count() > 0:
            raise forms.ValidationError(u"%s já pediu sua amizade" % to_user.username)
    ...
    def save(self):
        ...
        self.user.message_set.create(message="Pedido de amizade enviado para %s" % to_user.username) # @@@ make link like notification
        return invitation
